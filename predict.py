...
attention_weights = graph.get_tensor_by_name("attention_weights:0")  # Add this line to get the attention weights tensor
...
prediction, attention_weights_val = sess.run([decoded, attention_weights],  # Fetch attention weights
                                             feed_dict={
                                                 input: image,
                                                 seq_len: seq_lengths,
                                                 rnn_keep_prob: 1.0,
...
plt.imshow(attention_weights_val[0], cmap='hot', interpolation='nearest')
